...
var floodFill = function (image, sr, sc, newColor) {
    function getNeighbors(node) {
        ...
        if (r < image.length - 1) result.push([r + 1, c]);
        if (c < image[0].length - 1) result.push([r, c + 1]);
        result = result.filter(item => !visited.has(item[0]+'-'+item[1]) && image[item[0]][item[1]] === sourceColor)
        return result
    }
    let queue = [];
    let visited = new Set();
