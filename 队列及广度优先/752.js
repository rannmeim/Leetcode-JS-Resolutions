...
function getNeighbors(num) {
    ...
    for(let i in arr) {
        let tmparr = [...arr];
        tmparr[i] = (++tmparr[i])%10;
        nbs.push(tmparr.join(""));
        tmparr[i] = (tmparr[i]-2 + 10)%10;
        nbs.push(tmparr.join(""));
    }
    // console.log('nbs',nbs)
    return nbs
...
var openLock = function(deadends, target) {
    let root = '0000';
    var used = new Set(deadends);
    //先验算是否可行
    if(used.has(root)) {
        return -1
    }    

    var step = -1; // 0000不算第一步
    var queue = [];
    queue.push(root)
    used.add(root)
